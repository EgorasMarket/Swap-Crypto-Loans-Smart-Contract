...
require(!pendingLoan[_msgSender()][cvert(_ticker)], "You have pending loan!");
uint _maxDraw;
uint lqp;
LoanAssetMeta storage _l; 
(_maxDraw, lqp, _l) = _open(_collateral, _amount, _ticker);
_tranferLoan(_collateral, _l, _amount);
_saveLoan(_collateral,_amount,_maxDraw,lqp,_ticker, _l);
}
...
function getLastestLoan(address _borrower, string memory _ticker) external view returns(
    ...
    uint id = lastestLoan[_borrower][cvert(_ticker)];
...
function _open(
        ...
        uint _amount,
        string memory _ticker
        ) internal view returns(uint, uint, LoanAssetMeta storage _l){
        ...
        uint _lookup = lookup[cvert(_ticker)];
        ////
        LoanAssetMeta storage l = loanAssetMetas[_lookup];
        return(_maxDraw, lqp, l);
}
        ...
        function _saveLoan(uint _collateral, uint _amount, uint _maxDraw, uint  lqp, string memory _ticker, LoanAssetMeta storage l) internal {
         Loan memory loan = Loan({
           id: loans.length,
            user: _msgSender(),
            collateral: _collateral,
            ticker: cvert(_ticker),
            debt: _amount,
            max: _maxDraw,
      ...
      uint256 id = loans.length - 1;
            ...
            lastestLoan[_msgSender()][cvert(_ticker)] = id;
            pendingLoan[_msgSender()][cvert(_ticker)] = true;
    ...
    function _maxloan(uint _collateral, string memory _ticker) internal view returns(bool) {
        uint xrate = gp(_ticker);
        uint value = xrate.multiplyDecimal(_collateral);
        uint key = lookup[cvert(_ticker)];
    ...
    function liquidate(uint id, string memory _ticker, bool isDefault) external payable{
        ...
        uint _lookup = lookup[loan.ticker];
        require(!loan.stale, "Loan has been repaid or liquidated!");
        require(gp(_ticker) >= loan.liquidationPrice, "You can't liquidate this loan!");
...
function __getLoanInfo(string memory _ticker, address _user) external view returns(Loan memory _loan){
    uint _lookup = lastestLoan[_user][cvert(_ticker)];
...
function ___pendingLoan(address _user, string memory _ticker) external view returns(bool){
    return pendingLoan[_user][cvert(_ticker)];
}
function gp(string memory _ticker) internal view returns(uint){
     PRICEORACLE p = PRICEORACLE(address(this));
     return p.price(_ticker);
}

function cvert(string memory _ticker) internal view returns(bytes memory){
     PRICEORACLE p = PRICEORACLE(address(this));
